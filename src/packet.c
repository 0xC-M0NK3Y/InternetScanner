...
void create_ip_header(IP_HEADER *ip_hdr, ipv4_t source_ip, ipv4_t dest_ip) {
...
void create_tcp_header(TCP_HEADER *tcp_hdr, port_t dest_port, port_t source_port) {
...
unsigned short calcul_tcp_checksum(IP_HEADER ip_hdr, TCP_HEADER tcp_hdr) {
...
void create_packet(packet_t *packet, ipv4_t source_ip, ipv4_t dest_ip, port_t dest_port, port_t source_port) {
    ...
    create_ip_header(&ip_hdr, source_ip, dest_ip); // mettre son ip ici
